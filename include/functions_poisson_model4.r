...
jacfast_mda <- function(wh, xmat, beta, parallel=TRUE, ncpar=NULL){
  ...
  axABprime_sums <- Apply(data = list(xmat, aABprime),
               ...
               ncores = ncpar)$output1
  # dim(axABprime_sums)
  dim_rename <- dim(axABprime_sums)
  names(dim_rename) <- c("ik", "is", "js", "jk")
  dim(axABprime_sums) <- dim_rename
  # dim(axABprime_sums)
  
  
  print("getting axBAprime ...")
